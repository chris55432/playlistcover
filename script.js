...
async function ensureFullRes(cover) {
  const fullPath = cover?.dataset?.fullSrc;
  if (!fullPath) return;
  const img = cover.querySelector("img");
  if (!img) return;
  if (img.dataset.hiresLoaded === "1") return;

  // If we're still on the thumbnail, swap to full-res and wait for decode/load
  if (img.src && img.src.includes("/thumbs/")) {
    cover.classList.add("hires-loading");
    img.src = fullPath;
    try {
      ...
      // ignore decode errors; browser will still display when ready
    }
    img.dataset.hiresLoaded = "1";
    cover.classList.remove("hires-loading");
  }
}

async function openActive(cover) {
  if (activeCover && activeCover !== cover) closeActive();

  // Ensure the enlarged/active view always uses full-res artwork
  await ensureFullRes(cover);
